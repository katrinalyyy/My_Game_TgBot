"""
API обработчики для администратора
"""
...
from .schemas import (
    CreateCategoryRequestSchema, CategoryResponseSchema, CategoriesListResponseSchema,
    CreateQuestionRequestSchema, QuestionResponseSchema, QuestionsListResponseSchema,
    GamesListResponseSchema, AdminResponseSchema, SystemStatsResponseSchema
)
from app.common.schemas import QuestionsQuerySchema, GamesQuerySchema


@docs(
    tags=["admin"],
    summary="Создать категорию",
    description="Создает новую категорию вопросов"
)
@request_schema(CreateCategoryRequestSchema())
...
@response_schema(CategoriesListResponseSchema())
...
@docs(
    ...
    description="Создает новый вопрос в указанной категории"
)
...
@docs(
    ...
    description="Возвращает список вопросов с фильтрацией"
)
@querystring_schema(QuestionsQuerySchema())
@response_schema(QuestionsListResponseSchema())
...
@docs(
    tags=["admin"],
    summary="Получить игровые сессии",
    description="Возвращает список всех игровых сессий"
)
@querystring_schema(GamesQuerySchema())
@response_schema(GamesListResponseSchema())
...
@response_schema(SystemStatsResponseSchema())
...
@docs(
    tags=["admin"],
    summary="Очистить завершенные игры",
    description="Удаляет старые завершенные игры для освобождения места"
)
@request_schema({
    "days_old": fields.Int(missing=30, validate=validate.Range(min=1, max=365))
})
@response_schema(AdminResponseSchema())
async def cleanup_old_games(request: web.Request):
    try:
        data = await request.json()
        ...
        async for session in store.database.get_session():
            ...
            deleted_count = await game_accessor.cleanup_old_games(days_old)
            
            return web.json_response({
                "success": True,
                "message": f"Удалено {deleted_count} старых игр"
            })
            
    except Exception as e:
        return web.json_response(
            {"success": False, "error": f"Ошибка очистки игр: {str(e)}"},
            status=500
        )
